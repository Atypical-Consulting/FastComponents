...
## HtmxResponseHeaders\.Redirect\(string\) Method

Can be used to do a client\-side redirect to a new location

```csharp
public FastComponents.Http.HtmxResponseHeaders Redirect(string url);
```
#### Parameters

<a name='FastComponents.Http.HtmxResponseHeaders.Redirect(string).url'></a>
