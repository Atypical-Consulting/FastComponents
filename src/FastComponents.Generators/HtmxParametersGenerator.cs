...
public class HtmxParametersGenerator : IIncrementalGenerator
{
    ...
    private static GeneratorTarget? GetSemanticTargetForGeneration(GeneratorSyntaxContext context)
    {
        ...
        bool skipDefaults = true; // default value
        if (attribute.NamedArguments.Length > 0)
        {
            ...
            if (skipDefaultsArg.Key != null && skipDefaultsArg.Value.Value is bool skipDefaultsValue)
            {
    ...
    private static void GenerateBuildQueryString(IndentedCodeBuilder builder, List<IPropertySymbol> properties, string className, bool skipDefaults)
    {
        ...
        builder.Braces(() =>
        {
            ...
            foreach (IPropertySymbol? property in properties)
            {
                ...
                if (propertyType.SpecialType == SpecialType.System_String)
                {
                    ...
                    builder.Braces(() =>
                    {
                        if (skipDefaults)
                        {
                            ...
                            builder.Indent(() => builder.Line($"parts.Add($\"{propertyName}={{System.Uri.EscapeDataString({propertyName})}}\");"));
                        }
                        else
                        {
                ...
                else if (propertyType.SpecialType == SpecialType.System_Boolean)
                {
                    if (skipDefaults)
                    {
                        builder.Braces(() =>
                        {
                            builder.Line($"var defaultValue = new {className}().{propertyName};");
                            builder.Line($"if ({propertyName} != defaultValue)");
                            builder.Indent(() => builder.Line($"parts.Add($\"{propertyName}={{{propertyName}.ToString().ToLowerInvariant()}}\");"));
                        });
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    builder.Indent(() => builder.Line($"parts.Add($\"{propertyName}={{System.Uri.EscapeDataString({propertyName}.ToString() ?? \"\")}}\");"));
                }
