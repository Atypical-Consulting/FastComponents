    ...
    where TState : class, new()
{
    ...
    public string Url(TState? newState = null)
    {
        ...
        string componentName = GetType().Name.Replace("Component", string.Empty).ToLowerInvariant();
    ...
    /// </summary>
    public string GetRoute() => $"/htmx/{GetType().Name.Replace("Component", string.Empty).ToLowerInvariant()}";
    ...
    private static string ToQueryString(object obj)
    {
        string[] properties = [.. obj.GetType().GetProperties()
            ...
            .Select(p => $"{p.Name}={Uri.EscapeDataString(p.GetValue(obj)?.ToString() ?? string.Empty)}")];
...
public abstract class SimpleHtmxComponent : ComponentBase
{
    ...
    public static string GetRoute<T>()
        where T : SimpleHtmxComponent
        => $"/htmx/{typeof(T).Name.Replace("Component", string.Empty).ToLowerInvariant()}";
}
